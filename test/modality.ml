...
module Helpers : sig
  val require_success : ?here:Stdlib.Lexing.position -> string -> unit
  val require_failure : ?here:Stdlib.Lexing.position -> string -> unit
end = struct
  let check_portable =
  ...
  let require_success ?(here = Stdlib.Lexing.dummy_pos) str =
  ...
  let require_failure ?(here = Stdlib.Lexing.dummy_pos) str =
    let str = cleanup_input str in
    match parse_and_lint str with
