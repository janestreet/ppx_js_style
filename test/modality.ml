...
module Helpers : sig
  val require_success : ?here:Stdlib.Lexing.position -> ?cr:CR.t -> string -> unit
  val require_failure : ?here:Stdlib.Lexing.position -> ?cr:CR.t -> string -> unit
end = struct
  let check_portable =
  ...
  let require_success ?(here = Stdlib.Lexing.dummy_pos) ?cr str =
  ...
  let require_failure ?(here = Stdlib.Lexing.dummy_pos) ?cr str =
    let str = cleanup_input str in
    match parse_and_lint str with
